...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("label-filter", "Filter sources managed by external-dns via label selector when listing all resources; currently supported by source types CRD, ingress, service, node and openshift-route").Default(defaultConfig.LabelFilter).StringVar(&cfg.LabelFilter)
	...
	app.Flag("aws-default-ttl", "When using the AWS provider, this value will be set for unconfigured TTL values instead of 0 (default: 300)").Int64Var(&cfg.AWSDefaultTTL)
